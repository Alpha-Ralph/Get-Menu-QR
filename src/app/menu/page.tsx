...
export default function MenuManagement() {
  ...
  const [user, setUser] = useState<{ name: string; email: string; slug: string; currency?: string; categories?: string[] } | null>(null)
  const [menuItems, setMenuItems] = useState<MenuItem[]>([])
  const [showAddModal, setShowAddModal] = useState(false)
  const [editingItem, setEditingItem] = useState<MenuItem | null>(null)
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null)
  const [categories, setCategories] = useState<string[]>(['Appetizers', 'Main Course', 'Desserts', 'Beverages', 'Specials'])
  ...
  const loadRestaurantData = async (slug: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setCategories(restaurantData.categories || ['Appetizers', 'Main Course', 'Desserts', 'Beverages', 'Specials'])
              ...
              <div key={item.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="text-lg font-bold text-indigo-600">{currencySymbol}{item.price.toFixed(2)}</span>
                  </div>
